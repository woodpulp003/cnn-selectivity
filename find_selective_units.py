...
def load_activations_and_labels(layer_name, image_info_path, n_per_class):
...
def identify_selective_units_strict(activations, labels, classes, target_class, alpha):
    """
    Identify selective neurons for `target_class` by ensuring they are selective across all pairwise comparisons.
    """
    ...
    for other_class in other_classes:
        ...
        reject, _ = multipletests(p_vals, alpha=alpha, method="fdr_bh")[:2]
...
def calculate_fraction_and_save_selective_units(layer_name, activations, labels, classes, alpha, output_dir):
    ...
    for cls in classes:
        indices = identify_selective_units_strict(activations, labels, classes, cls, alpha)
...
if __name__ == "__main__":
    ...
    output_dir = "selective_analysis_output_0.01"
